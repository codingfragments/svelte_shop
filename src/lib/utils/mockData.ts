...
export function generateMockReviews(
	...
	const titles = {
		...
		switches: [
			...
			'Perfect tactile bump',
			'Great sound profile',
			'Consistent feel',
			'No wobble at all',
			'Love the actuation force',
			"Best switches I've tried",
			'Buttery smooth',
			'Perfect for typing'
		],
		keycaps: [
...
export function generateMockSpecs(category: string, productName: string): MockProductSpecs {
	...
	switch (category) {
		case 'keyboards':
			...
			specs['Mounting Style'] = ['Gasket', 'Top Mount', 'Tray Mount', 'Plateless'][
				Math.floor(Math.random() * 4)
			];
			specs['Case Material'] = ['Aluminum', 'Polycarbonate', 'ABS Plastic'][
				Math.floor(Math.random() * 3)
			];
			specs['Plate Material'] = ['FR4', 'Aluminum', 'Carbon Fiber', 'Polycarbonate'][
				Math.floor(Math.random() * 4)
			];
			specs['Keycaps'] = ['PBT Double-shot', 'ABS Double-shot', 'PBT Dye-sub'][
				Math.floor(Math.random() * 3)
			];
		...
		case 'keycaps':
			...
			specs['Legends'] = ['Double-shot', 'Dye-sublimated', 'Laser Etched'][
				Math.floor(Math.random() * 3)
			];
			specs['Thickness'] = `${(1.2 + Math.random() * 0.4).toFixed(1)}mm`;
			specs['Compatibility'] = 'Cherry MX style switches';
			specs['Layout Support'] = ['60%', '65%', '75%', 'TKL', 'Full Size']
		...
		case 'cables':
			specs['Length'] = [
				`${Math.floor(4 + Math.random() * 4)}ft`,
				`${Math.floor(6 + Math.random() * 6)}ft coiled`
		...
		case 'accessories':
			...
			specs['Dimensions'] =
				`${Math.floor(10 + Math.random() * 20)}cm x ${Math.floor(8 + Math.random() * 15)}cm`;
...
export function generateEnthusiastComment(
	productId: number,
	category: string,
	productName: string
): string {
